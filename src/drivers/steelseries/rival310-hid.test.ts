...
import { SteelSeriesRival310HidClient } from "./rival310-hid.ts";
...
function fakeDevice(options: { productId?: number; answerFirmware?: boolean; firmware?: number[] } = {}) {
  ...
  const device = {
    vendorId: 0x1038,
    productId: options.productId ?? 0x1720,
    productName: "SteelSeries Rival 310",
    opened: true,
    collections: [],
    open: async () => {},
    close: async () => {},
    sendReport: async (reportId: number, data: BufferSource) => {
      ...
      if (payload[0] === 0x90 && payload[1] === 0x00 && options.answerFirmware !== false) {
        const response = new Uint8Array(options.firmware ?? [0x00, 0x25]);
        queueMicrotask(() =>
...
test("claims all three Rival 310 product ids and rejects other SteelSeries PIDs", () => {
  const { device } = fakeDevice();
  assert.equal(SteelSeriesRival310HidClient.isSupported(device), true);
  assert.equal(SteelSeriesRival310HidClient.isSupported({ ...device, productId: 0x171e } as HIDDevice), true);
  assert.equal(SteelSeriesRival310HidClient.isSupported({ ...device, productId: 0x1736 } as HIDDevice), true);
  // Every sibling-cluster SteelSeries PID must not be claimed.
  for (const pid of [
    ...
    assert.equal(SteelSeriesRival310HidClient.isSupported({ ...device, productId: pid } as HIDDevice), false, `pid 0x${pid.toString(16)}`);
  }
  assert.equal(SteelSeriesRival310HidClient.isSupported({ ...device, vendorId: 0x1532 } as HIDDevice), false);
});

test("readStatus probes firmware and never claims to have read settings", async () => {
  const { device, sent } = fakeDevice();
  const status = await new SteelSeriesRival310HidClient(device).readStatus();
  assert.deepEqual(sent, [{ reportId: 0, payload: [0x90, 0x00] }]);
  assert.equal(status.brand, "SteelSeries");
  assert.equal(status.name, "SteelSeries Rival 310");
  assert.deepEqual(status.firmware, ["0.37"]);
...
test("a silent interface fails the probe loudly and names SteelSeries GG", async () => {
  ...
  await assert.rejects(new SteelSeriesRival310HidClient(device).readStatus(), /SteelSeries GG/);
  assert.deepEqual(sent, [{ reportId: 0, payload: [0x90, 0x00] }]);
});

test("setDpi and setPollingRate write the value then save, on report id 0", async () => {
  const { device, sent } = fakeDevice();
  const client = new SteelSeriesRival310HidClient(device);
  assert.equal(await client.setDpi(1600), 1600);
  assert.equal(await client.setPollingRate(500), 500);
  assert.deepEqual(sent, [
    { reportId: 0, payload: [0x53, 0x00, 0x01, 0x0f, 0x00, 0x42] },
...
test("setLedColor sends a feature report (not sendReport), then saves via output report", async () => {
  ...
  const client = new SteelSeriesRival310HidClient(device);
  await client.setLedColor("logo", 0xff, 0x00, 0x00);
...
test("setButtonsMapping writes the packet then saves", async () => {
  ...
  const packet = new Array(30).fill(0x00);
  packet[0x19] = 0x30;
  assert.deepEqual(sent, [
...
test("invalid values are rejected before any report reaches the mouse", async () => {
  ...
  await assert.rejects(client.setLedColor("logo", 300, 0, 0));
  assert.deepEqual(sent, []);
...
test("concurrent setters never interleave their write/save pairs", async () => {
  ...
  assert.deepEqual(sent.map(({ payload }) => payload[0]), [0x53, 0x59, 0x54, 0x59]);
});
