...
import { SteelSeriesAerox9WirelessHidClient } from "./aerox9-wireless-hid.ts";
...
function fakeDevice(
  ...
  const device = {
    vendorId: 0x1038,
    productId: options.productId ?? 0x185a, // wired-mode by default
    productName: "SteelSeries Aerox 9 Wireless",
    opened: true,
    collections: [],
    open: async () => {},
    close: async () => {},
    ...
    sendFeatureReport: async () => { throw new Error("the Aerox 9 Wireless protocol does not use feature reports"); },
    receiveFeatureReport: async () => { throw new Error("the Aerox 9 Wireless protocol does not use feature reports"); },
...
test("claims every Aerox 9 Wireless PID (both transports and the WOW edition) and nothing else", () => {
  const { device } = fakeDevice();
  for (const productId of [0x185a, 0x1876, 0x1858, 0x1874]) {
    assert.equal(
      SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId } as HIDDevice),
      true,
  ...
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1824 } as HIDDevice), false); // Rival 3
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x184c } as HIDDevice), false); // Rival 3 (new fw)
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1836 } as HIDDevice), false); // Aerox 3
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1830 } as HIDDevice), false); // Rival 3 Wireless
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1850 } as HIDDevice), false); // plain Aerox 5
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1854 } as HIDDevice), false); // Aerox 5 Wireless (wired)
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x185e } as HIDDevice), false); // Aerox 5 Wireless Destiny 2 (wired)
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1862 } as HIDDevice), false); // Aerox 5 Wireless Diablo IV (wired)
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1852 } as HIDDevice), false); // Aerox 5 Wireless (2.4 GHz)
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x185c } as HIDDevice), false); // Aerox 5 Wireless Destiny 2 (2.4 GHz)
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, productId: 0x1860 } as HIDDevice), false); // Aerox 5 Wireless Diablo IV (2.4 GHz)
  assert.equal(SteelSeriesAerox9WirelessHidClient.isSupported({ ...device, vendorId: 0x1532 } as HIDDevice), false);
});

test("readStatus probes battery (wired-mode command byte) and never claims to have read DPI/polling", async () => {
  const { device, sent } = fakeDevice({ productId: 0x185a });
  const status = await new SteelSeriesAerox9WirelessHidClient(device).readStatus();
...
test("readStatus in 2.4 GHz mode probes battery with the flagged command byte", async () => {
  const { device, sent } = fakeDevice({ productId: 0x1858 });
  const status = await new SteelSeriesAerox9WirelessHidClient(device).readStatus();
...
test("a silent interface fails the battery probe loudly and names SteelSeries GG", async () => {
  ...
  await assert.rejects(new SteelSeriesAerox9WirelessHidClient(device).readStatus(), /SteelSeries GG/);
  assert.deepEqual(sent, [{ reportId: 0, payload: [0x92] }]);
});

test("setDpi and setPollingRate write the value then save, wired-mode bytes", async () => {
  const { device, sent } = fakeDevice({ productId: 0x185a });
  const client = new SteelSeriesAerox9WirelessHidClient(device);
...
test("setZoneColor and setSleepTimer write the packet then save", async () => {
  const { device, sent } = fakeDevice({ productId: 0x185a });
  const client = new SteelSeriesAerox9WirelessHidClient(device);
